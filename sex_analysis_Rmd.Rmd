...
title: |
  ...
  | on sex differences in preen oil composition
...
knitr::opts_chunk$set(warning=FALSE, message=FALSE) # settings for the whole documents: no message and no warning will be displayed
```
...
setwd("C:/Users/localadmin/Documents/PhD SMELLY PLOVERS/Review/Collaborative review/sex differences/analysis")
```
...
Import data
```{r}
df <- read.csv("data_sex.csv")
```

Sample size = number of occurences (1 occurence = 1 season for a species)
```{r}
nrow(df)
```
...
```{r}
ss.sex <- data.frame(table(df$SexDiff)["0"],table(df$SexDiff)["1"])
colnames(ss.sex) <- c("no","yes")
rownames(ss.sex) <- c("sex difference")
ss.sex

ss.inc <- data.frame(table(df$Inc)["0"],table(df$Inc)["1"])
colnames(ss.inc) <- c("biparental","uniparental")
rownames(ss.inc) <- c("incubation type")
ss.inc

ss.season <- data.frame(table(df$Season)["0"],table(df$Season)["1"])
colnames(ss.season) <- c("nonbreeding","breeding")
rownames(ss.season) <- c("season")
ss.season
```
...
# species listed in the tree but not in our data (df = data with sex differences)

# Trim tree to have only species where sex differences are known
...
```{r, cache=TRUE}
# Model 1
mcmc1 <- MCMCglmm(SexDiff ~ Season + Inc, 
                  random = ~Phylo+Species,
                  family = "categorical", 
                  data = df, 
                  prior = prior2, 
                  ginverse =  list(Phylo = inv.phylo$Ainv),
                  nitt=10000000,burnin=5000,thin=2000,
                  verbose = FALSE)
```
...
Autocorrelation quickly curves to almost nothing.

View traces to make sure there are no patterns of autocorrelation
...
Plot trace and density of the inferred intercepts and variances 
...
Graphs look like a "hairy caterpillar", suggesting good convergence.
...
```{r, cache=TRUE}
# Model 2
mcmc2 <- MCMCglmm(SexDiff ~ Season + Inc, 
                  random = ~Phylo+Species,
                  family = "categorical", 
                  data = df, 
                  prior = prior2, 
...
# Model 3
mcmc3 <- MCMCglmm(SexDiff ~ Season + Inc, 
                  random = ~Phylo+Species,
                  family = "categorical", 
                  data = df, 
                  prior = prior2, 
...
Gelman-Rubin statistics (the potential scale reduction factor, PSRF)
(should be less than 1.1 among chains)
...
The three runs give the same results but values differ slightly. The second and third models were run only for verification purposes (Gelman-Rubin statistics). We decide to report only the results of the first model (i.e. mcmc1).
...
Plot the posterior distribution as a histogram to check for significance and whether it has been well estimated or not. 
...
mcmc.phylo <- MCMCglmm(SexDiff ~ Season + Inc, 
                  random = ~Phylo,
                  family = "categorical", 
                  data = df, 
                  prior = prior1, 
...
```{r, cache=TRUE}
mcmc.species <- MCMCglmm(SexDiff ~ Season + Inc, 
                  random = ~Species,
                  family = "categorical", 
                  data = df, 
                  prior = prior1, 
...
mcmc.norandom <- MCMCglmm(SexDiff ~ Season + Inc,
                  family = "categorical", 
                  data = df, 
                  prior = prior0, 
                  ginverse =  list(Phylo = inv.phylo$Ainv),
